...
public class MainViewModel {
	...
	private void addEntriesToCalendar() {
		...
		for (Appointment appointment : appointmentList) {
			...
			System.out.println("!!! TÄMÄKÖ SE VITTU KUSEE STAANA \n" + appointment.getProductList().get(0).getName());
	...
	public Appointment createAppointment(ZonedDateTime zdtStart, ZonedDateTime zdtEnd, Customer selectedCustomer,
			Animal animal, Product product, ShoppingCart shoppingCart, TextArea description) {
		...
		Appointment appointment = new Appointment(zdtStart, zdtEnd,
				selectedCustomer.getName() + " " + product.getName());
		appointment.addCustomer(selectedCustomer);
		appointment.addAnimal(animal);

		appointment.addProduct(product);
		System.out.println("VITUN PRODUCT\n == " + appointment.getProductList().get(0).getName());
		
		Map<Product, String> productsMap = shoppingCart.getProductList();
		for (Map.Entry<Product, String> entry : productsMap.entrySet()) {
			if (entry.getKey() != product)
			appointment.addProduct(entry.getKey());
			System.out.println("SHOPPINISSA == \n" + entry.getKey().getName());
		}
		System.out.println("VITUN PRODUCT2!!!!!!!!!!!!\n == " + appointment.getProductList().get(0).getName());

		appointment.setDescription(description.getText() != null ? description.getText() : "No description");
		appointmentDao.addAppointment(appointment);
		System.out.println("addAppointmentE: " + " " + zdtStart.toString() + " " + product.getName() + " "
				+ appointment.getId() + " \n");
		updateCalendar(true);
		return appointment;
	}
